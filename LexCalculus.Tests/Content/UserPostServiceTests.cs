using FluentAssertions;
using LexCalculus.Core.Entities.Content;
using LexCalculus.Core.Entities.Identity;
using LexCalculus.Core.Services;
using LexCalculus.Infrastructure.Data;
using LexCalculus.Infrastructure.Services;
using LexCalculus.Tests.TestHelpers;
using Microsoft.EntityFrameworkCore;
using Xunit;

namespace LexCalculus.Tests.Content;

public class UserPostServiceTests : SqlServerTestBase
{
    /// <summary>
    /// Test sanitizer — Program.cs'teki konfig ile aynı (sıkı whitelist).
    /// Default HtmlSanitizer style attribute'a izin veriyor; bu testlerde
    /// production davranışını test ediyoruz, default kullanılamaz.
    /// </summary>
    private static Ganss.Xss.HtmlSanitizer CreateSanitizer()
    {
        var s = new Ganss.Xss.HtmlSanitizer();
        s.AllowedTags.Clear();
        s.AllowedTags.UnionWith(new[]
        {
            "p", "br", "strong", "em", "u", "s",
            "h2", "h3", "h4",
            "ul", "ol", "li",
            "a", "blockquote", "code", "pre",
            "img", "figure", "figcaption"
        });
        s.AllowedAttributes.Clear();
        s.AllowedAttributes.UnionWith(new[]
        {
            "href", "title", "src", "alt", "width", "height", "class"
        });
        s.AllowedCssProperties.Clear();
        s.AllowedAtRules.Clear();
        s.AllowedSchemes.Clear();
        s.AllowedSchemes.UnionWith(new[] { "http", "https", "mailto" });
        return s;
    }

    // Adım 5.8 P2 — InMemory'den SQL Server'a geçiş: explicit Id atanamaz.
    // Seed edilen kategorilerin DB-tarafından üretilmiş Id'leri burada
    // tutulur; Input() default'u aktif kategoriyi kullanır.
    private int _activeCategoryId;
    private int _inactiveCategoryId;

    private (UserPostService svc, ApplicationDbContext ctx, PostTagService tagSvc) Setup(
        bool seedCategory = true)
    {
        var ctx = _db.Create();
        var tagSvc = new PostTagService(ctx);
        var sanitizer = CreateSanitizer();
        var svc = new UserPostService(ctx, tagSvc, new NullActivityLogService(), sanitizer);
        if (seedCategory)
        {
            var active = new PostCategory
            {
                Name = "Genel", Slug = "genel", DisplayOrder = 1,
                IsActive = true, CreatedAt = DateTime.UtcNow
            };
            var inactive = new PostCategory
            {
                Name = "Pasif", Slug = "pasif", DisplayOrder = 2,
                IsActive = false, CreatedAt = DateTime.UtcNow
            };
            ctx.PostCategories.Add(active);
            ctx.PostCategories.Add(inactive);
            ctx.SaveChanges();
            _activeCategoryId = active.Id;
            _inactiveCategoryId = inactive.Id;
        }
        return (svc, ctx, tagSvc);
    }

    private static async Task<int> SeedUserAsync(ApplicationDbContext ctx, string discriminator)
    {
        var user = new ApplicationUser
        {
            UserName = $"u{discriminator}@x.com", NormalizedUserName = $"U{discriminator}@X.COM",
            Email = $"u{discriminator}@x.com", NormalizedEmail = $"U{discriminator}@X.COM",
            FullName = $"User {discriminator}", CreatedAt = DateTime.UtcNow,
            IsActive = true, EmailConfirmed = true,
            SecurityStamp = Guid.NewGuid().ToString()
        };
        ctx.Users.Add(user);
        await ctx.SaveChangesAsync();
        return user.Id;
    }

    private UserPostInput Input(string title = "Test Makalesi",
        string body = "<p>içerik</p>", int? categoryId = null,
        IReadOnlyList<string>? tags = null)
        => new(title, body, categoryId ?? _activeCategoryId, null, tags ?? Array.Empty<string>());

    // ─── CreateDraft ──────────────────────────────────────────────────────

    [Fact]
    public async Task CreateDraft_Valid_GeneratesSlug()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input("İş Hukuku Makalem"));

        result.Success.Should().BeTrue();
        result.Post!.Slug.Should().Be("is-hukuku-makalem");
        result.Post.IsPublished.Should().BeFalse();
        result.Post.PublishedAt.Should().BeNull();
    }

    [Fact]
    public async Task CreateDraft_DuplicateTitleSameUser_AppendsSuffix()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        await svc.CreateDraftAsync(userId, Input("Aynı Başlık"));
        var second = await svc.CreateDraftAsync(userId, Input("Aynı Başlık"));
        var third = await svc.CreateDraftAsync(userId, Input("Aynı Başlık"));

        second.Post!.Slug.Should().Be("ayni-baslik-2");
        third.Post!.Slug.Should().Be("ayni-baslik-3");
    }

    [Fact]
    public async Task CreateDraft_SameTitleDifferentUsers_AllowedInUserNamespace()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var user2Id = await SeedUserAsync(ctx, "2");

        var u1 = await svc.CreateDraftAsync(userId, Input("Aynı Başlık"));
        var u2 = await svc.CreateDraftAsync(user2Id, Input("Aynı Başlık"));

        u1.Post!.Slug.Should().Be("ayni-baslik");
        u2.Post!.Slug.Should().Be("ayni-baslik",
            "user namespace altında çakışma yok");
    }

    [Fact]
    public async Task CreateDraft_EmptyTitle_ReturnsError()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input("   "));

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("Başlık");
    }

    [Fact]
    public async Task CreateDraft_EmptyBody_ReturnsError()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(body: " "));

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("İçerik");
    }

    [Fact]
    public async Task CreateDraft_InactiveCategory_ReturnsError()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(categoryId: _inactiveCategoryId));

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("aktif değil");
    }

    [Fact]
    public async Task CreateDraft_TooManyTags_ReturnsError()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(
            tags: new[] { "a", "b", "c", "d", "e", "f" }));

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("etiket");
    }

    [Fact]
    public async Task CreateDraft_TagsAdded_DraftDoesNotIncrementUsageCount()
    {
        var (svc, ctx, tagSvc) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk", "iş hukuku" }));

        var tags = await ctx.PostTags.AsNoTracking().ToListAsync();
        tags.Should().HaveCount(2);
        tags.Should().AllSatisfy(t => t.UsageCount.Should().Be(0,
            "draft state'te tag UsageCount artmaz"));
    }

    // ─── Update ───────────────────────────────────────────────────────────

    [Fact]
    public async Task Update_NonOwner_ReturnsUnauthorized()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var user2Id = await SeedUserAsync(ctx, "2");
        var draft = await svc.CreateDraftAsync(userId, Input());

        var result = await svc.UpdateAsync(draft.Post!.Id, actingUserId: user2Id, Input("Yeni"));

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("yetk");
    }

    [Fact]
    public async Task Update_DoesNotRegenerateSlug()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input("İlk Başlık"));
        var originalSlug = draft.Post!.Slug;

        var updated = await svc.UpdateAsync(draft.Post.Id, userId, Input("Tamamen Farklı Başlık"));

        updated.Success.Should().BeTrue();
        updated.Post!.Title.Should().Be("Tamamen Farklı Başlık");
        updated.Post.Slug.Should().Be(originalSlug,
            "Yaklaşım 4: slug ilk üretimde sabit");
    }

    [Fact]
    public async Task Update_TagsChanged_PublishedPost_AdjustsUsageCount()
    {
        var (svc, ctx, tagSvc) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk", "iş hukuku" }));
        await svc.PublishAsync(draft.Post!.Id, userId);

        // Yayında: kvkk=1, iş hukuku=1
        // Update ile kvkk çıkar, ticaret ekle
        await svc.UpdateAsync(draft.Post.Id, userId, Input(tags: new[] { "iş hukuku", "ticaret" }));

        var kvkk = await ctx.PostTags.AsNoTracking().FirstAsync(t => t.Slug == "kvkk");
        var isHukuku = await ctx.PostTags.AsNoTracking().FirstAsync(t => t.Slug == "is-hukuku");
        var ticaret = await ctx.PostTags.AsNoTracking().FirstAsync(t => t.Slug == "ticaret");

        kvkk.UsageCount.Should().Be(0, "kvkk kaldırıldı");
        isHukuku.UsageCount.Should().Be(1, "iş hukuku korundu");
        ticaret.UsageCount.Should().Be(1, "ticaret eklendi");
    }

    // ─── Publish ──────────────────────────────────────────────────────────

    [Fact]
    public async Task Publish_FromDraft_SetsPublishedAtAndIncrementsTagCount()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk" }));

        var result = await svc.PublishAsync(draft.Post!.Id, userId);

        result.Success.Should().BeTrue();
        result.Post!.IsPublished.Should().BeTrue();
        result.Post.PublishedAt.Should().NotBeNull();
        var tag = await ctx.PostTags.AsNoTracking().FirstAsync(t => t.Slug == "kvkk");
        tag.UsageCount.Should().Be(1);
    }

    [Fact]
    public async Task Publish_AlreadyPublished_NoOp()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk" }));
        await svc.PublishAsync(draft.Post!.Id, userId);

        var second = await svc.PublishAsync(draft.Post.Id, userId);

        second.Success.Should().BeTrue();
        var tag = await ctx.PostTags.AsNoTracking().FirstAsync(t => t.Slug == "kvkk");
        tag.UsageCount.Should().Be(1, "ikinci publish çağrısı tag count artırmaz");
    }

    // ─── Unpublish ────────────────────────────────────────────────────────

    [Fact]
    public async Task Unpublish_FromPublished_DecrementsTagCount()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk" }));
        await svc.PublishAsync(draft.Post!.Id, userId);

        await svc.UnpublishAsync(draft.Post.Id, userId);

        var tag = await ctx.PostTags.AsNoTracking().FirstAsync(t => t.Slug == "kvkk");
        tag.UsageCount.Should().Be(0);
        var refreshed = await ctx.UserPosts.AsNoTracking().FirstAsync(p => p.Id == draft.Post.Id);
        refreshed.IsPublished.Should().BeFalse();
    }

    // ─── Delete ───────────────────────────────────────────────────────────

    [Fact]
    public async Task Delete_PublishedPost_DecrementsTagCountAndCascadesLinks()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk", "ticaret" }));
        await svc.PublishAsync(draft.Post!.Id, userId);

        var result = await svc.DeleteAsync(draft.Post.Id, userId);

        result.Success.Should().BeTrue();
        (await ctx.UserPosts.AnyAsync(p => p.Id == draft.Post.Id)).Should().BeFalse();
        (await ctx.PostTagLinks.AnyAsync(l => l.PostId == draft.Post.Id))
            .Should().BeFalse("cascade ile link satırları silinir");
        var tags = await ctx.PostTags.AsNoTracking().ToListAsync();
        tags.Should().AllSatisfy(t => t.UsageCount.Should().Be(0));
    }

    [Fact]
    public async Task Delete_DraftPost_DoesNotAffectTagCount()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input(tags: new[] { "kvkk" }));

        await svc.DeleteAsync(draft.Post!.Id, userId);

        var tag = await ctx.PostTags.AsNoTracking().FirstOrDefaultAsync(t => t.Slug == "kvkk");
        tag.Should().NotBeNull();
        tag!.UsageCount.Should().Be(0, "draft silindi, tag zaten 0'dı");
    }

    [Fact]
    public async Task Delete_NonOwner_ReturnsUnauthorized()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var user2Id = await SeedUserAsync(ctx, "2");
        var draft = await svc.CreateDraftAsync(userId, Input());

        var result = await svc.DeleteAsync(draft.Post!.Id, actingUserId: user2Id);

        result.Success.Should().BeFalse();
    }

    // ─── Query ────────────────────────────────────────────────────────────

    [Fact]
    public async Task GetByUserIdAsync_IncludeUnpublishedTrue_ReturnsBoth()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input("Birinci"));
        var second = await svc.CreateDraftAsync(userId, Input("İkinci"));
        await svc.PublishAsync(second.Post!.Id, userId);

        var list = await svc.GetByUserIdAsync(userId, includeUnpublished: true);

        list.Should().HaveCount(2);
    }

    [Fact]
    public async Task GetByUserIdAsync_IncludeUnpublishedFalse_OnlyPublished()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        await svc.CreateDraftAsync(userId, Input("Taslak"));
        var pub = await svc.CreateDraftAsync(userId, Input("Yayında"));
        await svc.PublishAsync(pub.Post!.Id, userId);

        var list = await svc.GetByUserIdAsync(userId, includeUnpublished: false);

        list.Should().HaveCount(1);
        list[0].Title.Should().Be("Yayında");
    }

    [Fact]
    public async Task GetByUserAndSlugAsync_ReturnsPostWithTagsAndCategory()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input("Test Post", tags: new[] { "kvkk" }));

        var fetched = await svc.GetByUserAndSlugAsync(userId, draft.Post!.Slug);

        fetched.Should().NotBeNull();
        fetched!.Category.Should().NotBeNull();
        fetched.TagLinks.Should().HaveCount(1);
        fetched.TagLinks.First().Tag.Slug.Should().Be("kvkk");
    }

    // ─── Sanitize (Faz 4.6 P3) ───────────────────────────────────────────

    [Fact]
    public async Task CreateDraft_BodyWithScript_StripsScriptTag()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(
            body: "<p>Güvenli</p><script>alert('xss')</script><p>Devam</p>"));

        result.Success.Should().BeTrue();
        result.Post!.Body.Should().NotContain("<script");
        result.Post.Body.Should().NotContain("alert");
        result.Post.Body.Should().Contain("Güvenli");
        result.Post.Body.Should().Contain("Devam");
    }

    [Fact]
    public async Task CreateDraft_BodyWithImg_KeepsImgWithSafeAttrs()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(
            body: "<p><img src=\"https://example.com/x.jpg\" alt=\"test\" /></p>"));

        result.Success.Should().BeTrue();
        result.Post!.Body.Should().Contain("<img");
        result.Post.Body.Should().Contain("src=\"https://example.com/x.jpg\"");
        result.Post.Body.Should().Contain("alt=\"test\"");
    }

    [Fact]
    public async Task CreateDraft_BodyWithStyleAttribute_StripsStyle()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(
            body: "<p style=\"color:red;background:yellow\">renkli</p>"));

        result.Success.Should().BeTrue();
        result.Post!.Body.Should().NotContain("style=");
        result.Post.Body.Should().Contain("renkli");
    }

    [Fact]
    public async Task CreateDraft_BodyWithJavascriptHref_StripsHref()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(
            body: "<p><a href=\"javascript:alert(1)\">tıkla</a></p>"));

        result.Success.Should().BeTrue();
        result.Post!.Body.ToLowerInvariant().Should().NotContain("javascript:");
    }

    [Fact]
    public async Task CreateDraft_BodyWithOnclickHandler_StripsHandler()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");

        var result = await svc.CreateDraftAsync(userId, Input(
            body: "<p onclick=\"alert('x')\">tıkla beni</p>"));

        result.Success.Should().BeTrue();
        result.Post!.Body.Should().NotContain("onclick");
        result.Post.Body.Should().NotContain("alert");
    }

    [Fact]
    public async Task UpdateAsync_BodyWithScript_StripsScriptTag()
    {
        var (svc, ctx, _) = Setup();
        var userId = await SeedUserAsync(ctx, "1");
        var draft = await svc.CreateDraftAsync(userId, Input("Original"));

        var updated = await svc.UpdateAsync(draft.Post!.Id, userId, Input(
            "Updated", body: "<p>safe</p><script>evil</script>"));

        updated.Success.Should().BeTrue();
        updated.Post!.Body.Should().NotContain("<script");
        updated.Post.Body.Should().NotContain("evil");
    }
}
